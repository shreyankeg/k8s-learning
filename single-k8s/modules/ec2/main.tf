locals {
  total = var.master_count + var.agent_count
  instance_names = concat(
    [for i in range(var.master_count) : "${var.instance_name_prefix}-master-${i + 1}"],
    [for i in range(var.agent_count) : "${var.instance_name_prefix}-agent-${i + 1}"]
  )
}
